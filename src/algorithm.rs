...
pub fn kmeans_triangle_inequality(
...
) -> Result<(Vec<Vec<u32>>, Vec<usize>), &'static str> {
    if k > data.len() {
    ...
    let final_labels = labels.iter().map(|x| x.load(Ordering::Relaxed)).collect();
    log::info!("Finished KMeans");
    return Ok((Arc::try_unwrap(centroids).unwrap().into_inner().unwrap(), final_labels))
}
...
pub fn kmeans_default(
    ...
    let final_labels = labels.iter().map(|x| x.load(Ordering::Relaxed)).collect();
    Ok((Arc::try_unwrap(centroids).unwrap().into_inner().unwrap(), final_labels))
}