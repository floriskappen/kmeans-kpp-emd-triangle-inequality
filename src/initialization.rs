...
pub fn kmeans_plusplus_euclidian(data: &[u8], histogram_size: usize, k: usize) -> Vec<Vec<f64>> {
    ...
    centroids.push(data[first_idx..first_idx + histogram_size].iter().map(|&val| val as f64).collect_vec());

    if k == 1 {
        return centroids;
    }

    let mut min_distances = vec![f64::MAX; num_histograms];
    ...
    for _ in 1..k {
        ...
        min_distances.par_iter_mut()
            ...
            .for_each(|(idx, min_dist)| {
                ...
                let histogram_as_64: Vec<f64> = data[start_index..start_index + histogram_size].iter().map(|&v| v as f64).collect();
        ...
        centroids.push(data[next_centroid_idx..next_centroid_idx + histogram_size].iter().map(|&val| val as f64).collect_vec());
    }

    centroids
}

pub fn kmeans_plusplus_emd(data: &[u8], histogram_size: usize, k: usize) -> Vec<Vec<f64>> {
    let mut rng = rand::thread_rng();
    let mut centroids: Vec<Vec<f64>> = Vec::new();
    let num_histograms = data.len() / histogram_size;
    ...
    // Randomly select the first centroid
    let first_idx = rng.gen_range(0..num_histograms) * histogram_size;
    centroids.push(data[first_idx..first_idx + histogram_size].iter().map(|&v| v as f64).collect());
...
pub fn kmeans_plusplus_emd_f64(data: &[f64], histogram_size: usize, k: usize) -> Vec<Vec<f64>> {
    let mut rng = rand::thread_rng();
    let mut centroids: Vec<Vec<f64>> = Vec::new();
    let num_histograms = data.len() / histogram_size;
    ...
    // Randomly select the first centroid
    let first_idx = rng.gen_range(0..num_histograms) * histogram_size;
    centroids.push(data[first_idx..first_idx + histogram_size].to_vec());
    ...
    for _ in 1..k {
        ...
        min_distances.par_iter_mut()
            ...
            .for_each(|(idx, min_dist)| {
                let start_index = idx * histogram_size;
                let distance = earth_movers_distance(
                    &data[start_index..start_index + histogram_size].to_vec(),
                    &centroids.last().unwrap()
                );
                *min_dist = (*min_dist).min(distance as f32);
            });
        ...
        let dist = WeightedIndex::new(&min_distances).unwrap();
        let next_centroid_idx = dist.sample(&mut rng);
        centroids.push(data[next_centroid_idx..next_centroid_idx + histogram_size].to_vec());
    }
